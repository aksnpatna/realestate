...
describe('BuyFinder', () => {
  beforeEach(() => {
    ...
    mockFetch.mockImplementation(async (url) => {
      if (url === '/api/clients') return { ok: true, json: async () => [] }
  ...
  it('renders search button and state selector', () => {
    render(<BuyFinder />)
    expect(screen.getByText('Calculate Buyer Fit & Search')).toBeInTheDocument()
  ...
  it('result shows View decision button', async () => {
    ...
    render(<BuyFinder />)
    const searchBtn = screen.getByText('Calculate Buyer Fit & Search')
    searchBtn.click()
