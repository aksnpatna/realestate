...
def _build() -> Dict[str, MetricEntry]:
    ...
    return {
        ...
        "price_volatility_10yr": e("price_volatility_10yr", "Price Volatility (10yr)", "%", "Derived (stdev of annual returns)", 45, "lower_better",
                                   col=None, calc="stdev(year-over-year % returns, 10yr)"),
        "price_sharpe_ratio":   e("price_sharpe_ratio",    "Risk-Adj. Return (Sharpe)", "ratio", "Derived (mean return / stdev)", 45, "higher_better",
                                   col=None, calc="mean(annual % returns) / stdev(annual % returns)"),
