# Graph Report - realestate  (2026-07-14)

## Corpus Check
- 145 files · ~2,262,041 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1341 nodes · 1812 edges · 173 communities (133 shown, 40 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 299 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b6d67ded`
...
5. `ModelDiary` - 30 edges
6. `SuburbUIV3` - 29 edges
...
Cohesion: 0.06
Nodes (53): analyze_suburb(), bust_suburbs_cache(), buy_finder_rank(), calculate_roi(), calculate_stamp_duty(), _check_auth_rate(), _check_rate_limit(), create_model_diary_entry() (+45 more)
...
Cohesion: 0.06
Nodes (40): Any, agent_router.py — Rules-based dynamic agent selection for the investment committ, Determine which committee agents to run for a given suburb.      Rules (evaluate, route_agents(), bear_agent_node(), bull_agent_node(), CommitteeState, CommitteeVerdict (+32 more)
...
## Knowledge Gaps
- **559 isolated node(s):** `$schema`, `plugins`, `react/rules-of-hooks`, `react/only-export-components`, `TabName` (+554 more)
...
- **Why does `V3Scheduler` connect `V3 Scheduler Orchestration` to `Community 153`?**
...
- **Are the 27 inferred relationships involving `ModelDiary` (e.g. with `ActivityRequest` and `AnalyzeRequest`) actually correct?**
  _`ModelDiary` has 27 INFERRED edges - model-reasoned connections that need verification._